function [y, iter, k, m, errest] = RKcompress_fAb_block(A, b, xi, f, tol, options)
	% function [y, iter, k, m, errest] = RKcompress_fAb_block(A, b, xi, f, tol, options)
	% Computes f(A)b for A Hermitian, b block vector employing the low-memory Lanczos algorithm 
	% with rational Krylov compression described in [1]
	% The algorithm checks convergence by monitoring the difference between consecutive
    % iterations 
	%
	% [1] A. A. Casulli , Block rational Krylov methods for matrix equations and matrix 
    %  functions, PhD thesis, 2024
	%
	% Input:
	...
	%	errest		matrix with two columns, each row contains an iteration number 
	...
	if (isstring(xi) || ischar(xi)) && xi == "markov"
		...
		if isfield (options, 'a')==0 || isfield (options, 'b')==0
			...
			[y, iter, k, m, errest] = RKcompress_matfun_block(mult, b, xi_MF, f, options);
		end
	elseif (isstring(xi) || ischar(xi)) && xi == "exp" % 25 poles for rational approximation of the exponential on the negative real axis
		...
		[y, iter, k, m, errest] = RKcompress_matfun_block(mult, b, xi_exp, f, options);
	else
		[y, iter, k, m, errest] = RKcompress_matfun_block(mult, b, xi, f, options);
	end	
